    ...
    if astData:
        ...
        if model_armor_config:
            ...
            if not model_armor_config.get("userPromptTemplate"):
                message_data["faults"].append(
                    {
                        "code": 101,
                        "reason": "MODELARMOR: No User Prompt Template configured",
                    }
                )
            if not model_armor_config.get("responseTemplate"):
                message_data["faults"].append(
                    {
                        "code": 102,
                        "reason": "MODELARMOR: No Response Template configured",
                    }
                )
            if str(model_armor_config.get("failureMode")) != "FAIL_CLOSED":
                message_data["faults"].append(
                    {"code": 103, "reason": "MODELARMOR: Failure Mode set incorrectly"}
                )
        else:
            message_data["faults"].append(
                {"code": 104, "reason": "MODELARMOR: Model Armor not configured"}
            )
    else:
        message_data["faults"].append(
            {
                "code": 105,
                "reason": f"ASSISTANT: Could not get assistant config for {engEndpoint}",
            }
        )
    return message_data
...
def get_cmek_data(api_endpoint, engProject, engLocation, headers):
    ...
    if cmekData and cmekData.get("kmsKey"):
        ...
        if cmekData.get("notebooklmState") != "NOTEBOOK_LM_READY":
            message_data["faults"].append(
                {"code": 202, "reason": "CMEK: notebooklmState is not ready"}
            )
        if cmekData.get("state") != "ACTIVE":
            message_data["faults"].append(
                {"code": 203, "reason": "CMEK: State is not active"}
            )
    else:
        message_data["faults"].append(
            {"code": 201, "reason": "CMEK: CMEK not configured"}
        )
...
def get_engine_data(api_endpoint, engEndpoint, headers):
    ...
    if engData and engData.get("features"):
        ...
        if feature_config.get("disable-video-generation") != "FEATURE_STATE_ON":
            message_data["faults"].append(
                {"code": 302, "reason": "FEATURES: disable-video-generation is not ON"}
            )
        if feature_config.get("disable-image-generation") != "FEATURE_STATE_ON":
            message_data["faults"].append(
                {"code": 303, "reason": "FEATURES: disable-image-generation is not ON"}
            )
        if feature_config.get("disable-talk-to-content") != "FEATURE_STATE_ON":
            message_data["faults"].append(
                {"code": 304, "reason": "FEATURES: disable-talk-to-content is not ON"}
            )
        if feature_config.get("bi-directional-audio") != "FEATURE_STATE_OFF":
            message_data["faults"].append(
                {"code": 305, "reason": "FEATURES: bi-directional-audio is not OFF"}
            )
    else:
        message_data["faults"].append(
            {"code": 301, "reason": "FEATURES: Could not get feature config"}
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pubsub_topic", help="The Pub/Sub topic to publish to.")
    ...
    for engine in get_engines(org_resource):
        ...
        if args.pubsub_topic:
            publish_to_pubsub(args.pubsub_topic, message_data)
        else:
